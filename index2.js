...
class SymetryCentrale{
    static start(){
        gameCanvas.addEventListener("click", SymetryCentrale.choose);
        gameCanvas.addEventListener("mousemove", SymetryCentrale.move);
    }
    static move(e) {
        gc.putImageData(imageZero, 0,0);
        if (sx && sy)
    ...
    static choose(e){
        sx=proximate(e.offsetX,e.offsetY).x;
        sy=proximate(e.offsetX,e.offsetY).y;
        gc.putImageData(imageZero, 0,0);
        redrawAll();
        point(sx,sy,"red",5);
    }
    static end(){
        gameCanvas.removeEventListener("click", SymetryCentrale.choose);
    }
}
...
function setUP(){
    ...
    document.getElementById("chooseCenter").addEventListener("click", function () {chooseEvent("chooseCenter")});
